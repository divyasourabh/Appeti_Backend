...
public class AesCryptUtil {
    ...
    */
    ...
    public static String encrypt(String plainText, String key){
		try{
			PublicKey publicKey = KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(AndroidBase64.decode(key, AndroidBase64.DEFAULT)));
		    Cipher cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
		    cipher.init(Cipher.ENCRYPT_MODE, publicKey);
		    return AndroidBase64.encodeToString(cipher.doFinal(plainText.getBytes("UTF-8")),AndroidBase64.DEFAULT);
		}catch (Exception e) {
			e.printStackTrace();
		}
		return null;

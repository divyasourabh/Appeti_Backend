...
jQuery(function(a) {
    ...
    a("body").on("change", "select.country_to_state, input.country_to_state", function() {
        var b = a(this).val(),
            c = a(this).closest("div").find("#billing_state, #shipping_state, #calc_shipping_state"),
            d = c.parent(),
            f = c.attr("name"),
            g = c.attr("id"),
            h = c.val(),
        ...
        if (e[b])
            if (a.isEmptyObject(e[b])) c.parent().hide().find(".select2-container").remove(), c.replaceWith('<input type="hidden" class="hidden" name="' + f + '" id="' + g + '" value="" placeholder="' + i + '" />'), a("body").trigger("country_to_state_changed", [b, a(this).closest("div")]);
            else {
                var j = "",
                    k = e[b];
